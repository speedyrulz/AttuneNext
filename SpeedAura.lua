...
--
-- The challenge time is a per-instance, server-set target, so it is used as
-- that instance's clear length when the Run length setting asks for personal
-- times. Nothing is timed or recorded by the addon itself.
--   db.challengeTimes["<map>:<difficulty>"] = seconds
-- Times are account-wide (AttuneNextDB is a per-account saved variable).
-- =========================================================================
...
-- Personal clear time for a run: the Dungeon Challenge target, if we have it.
...
function ANx.ScanSpeedAuras()
    ...
    if prev and math.abs(prev - challenge) < 1 then return end
...
function ANx.PrintChallengeTimes()
    local t = (ANx.db and ANx.db.challengeTimes) or {}
    local rows = {}
    for key, sec in pairs(t) do
        ...
        rows[#rows + 1] = { name = name .. ((label ~= "") and (" (" .. label .. ")") or ""), sec = sec }
    end
    table.sort(rows, function(a, b) return a.sec < b.sec end)
    if #rows == 0 then
        ANx.Print("No Dungeon Challenge times seen yet - they are read from the buff when you enter an instance.")
        return
    end
    ANx.Print("Dungeon Challenge times (1.00x = " .. ANx.FormatRunTime(ANx.BASE_RUN_SECONDS) .. "):")
